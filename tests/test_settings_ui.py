...
class TestSettingsUI(unittest.TestCase):
    ...
    def test_pause_listener_mode2_and_mode1_playback_state(self):
        """audioPlayer pause event listener sets playing for Mode 2 (user pause or in-call) and paused for Mode 1 or media focus change"""
        pause_block_match = re.search(r'audioPlayer\.addEventListener\(\s*[\'"]pause[\'"]\s*,[\s\S]*?\}\);', self.main_content)
        self.assertIsNotNone(pause_block_match, "Could not find audioPlayer pause listener in main.js")
        pause_block = pause_block_match.group(0)
        self.assertRegex(
            pause_block,
            r'if\s*\(\s*window\.playbackMode\s*===\s*[\'"]mode2[\'"]\s*&&\s*\(window\.wasPausedByUser\s*\|\|\s*window\.isCallActive\)\s*&&\s*!isRecentBtDisconnect\s*\)\s*\{[\s\S]*?playbackState\s*=\s*[\'"]playing[\'"]\s*;?[\s\S]*?\}\s*else\s*\{[\s\S]*?playbackState\s*=\s*[\'"]paused[\'"]\s*;?[\s\S]*?\}'
        )
